...
public class KjedetOrdnetListe<T extends Comparable<T>> implements OrdnetListeADT<T> {
	...
	private LinearNode<T> forrige(T element) {

		LinearNode<T> node = foerste;
		LinearNode<T> forrige = null;

		while ((node != null) && (element.compareTo(node.getElement()) > 0)) {
			forrige = node;
			node = node.getNeste();
		}
		return forrige;
	...
	public void leggTil(T element) {
		...
		LinearNode<T> forrige = forrige(element);
		...
		if (erTom() || (forrige == null)) {
			...
			LinearNode<T> neste = forrige.getNeste();
			ny.setNeste(neste);
			forrige.setNeste(ny);
